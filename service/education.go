...
func (education) List(page *model.Page, education *model.Education, users *[]model.Education) error {
	if err := cs.Sql.Table("project").Where("dtd=false").Limit(page.Limit(), page.Skip()).Find(users, education); err != nil {
		return err
	}
	return nil
}
func (education) Delete(education *model.Education) error {
	education.Dtd = true
	if _, err := cs.Sql.Table("project").Update(education); err != nil {
		return err
	}
	return nil
}
func (education) Get(education *model.Education) error {
	...
	if _, err := cs.Sql.Table("project").ID(education.Id).Update(education); err != nil {
		return err
	}
	return nil
}
